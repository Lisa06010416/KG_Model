...
class creatModel(baseModel):
    def __init__(self, pars, Data):
        ...
        self.buildModel()
        # data generate
        self.TransE_DataGenerator = Triplet_DataGenerator(dataset = self.Data.triplet_train, 
    ...
    def createEmbedding(self):
        ...
        self.R_E = Embedding(self.Data.max_r, self.emSize, embeddings_regularizer=regularizers.l2(self.alpha), embeddings_initializer = initializers.RandomUniform(minval=-sqrt_size, maxval=sqrt_size), name="R1_E", input_length=1)
        self.E_E = Embedding(self.Data.max_e, self.emSize, embeddings_regularizer=regularizers.l2(self.alpha), embeddings_initializer = initializers.RandomUniform(minval=-sqrt_size, maxval=sqrt_size), name="Eh_E", input_length=1)
    ...
    def buildTransE(self):
        ...
        self.predictModel = Model([hp, r ,tp], i_pos_dissims, name='TransE_predictModel')  
        
    def training(self):
        if self.Data.isValid == 0 :
            history = self.model.fit_generator( generator=self.TransE_DataGenerator, 
                                                epochs=1, 
                                                verbose=1, 
        ...
        self.getHistory(test = {"testTransE":testTransE}, 
                                history = history, 
                                modelName = "TransE", 
                                recordList = ['loss', 'val_loss', 'calAccuracy', 'val_calAccuracy', "testTransE"])
        return self.model
